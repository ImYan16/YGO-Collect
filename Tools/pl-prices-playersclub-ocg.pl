...
# Harvest Players Club OCG singles for GitHub Actions.
# Run from the repository root:
#   perl Tools/pl-prices-playersclub-ocg.pl
...
sub parse_title {
  ...
  return unless $raw =~ /^\s*([A-Z0-9]{2,12}-JP[A-Z0-9]{1,6})\s*(.*)$/i;
  my ($code, $rest) = (uc($1), $2);
  ...
  my $rarity = '';
  $rarity = $1 if $without_status =~ /\(\s*([A-Za-z][A-Za-z'’._\/ -]*(?:\s+[A-Za-z][A-Za-z'’._\/ -]*)*)\s*\)/i;
  $rarity =~ s/^\s+|\s+$//g if $rarity;
  $rarity = uc($rarity) if $rarity;

  $rest =~ s/\(\s*Status[^)]*\)//gi;
  $rest =~ s/\(\s*[A-Za-z][A-Za-z'’._\/ -]*(?:\s+[A-Za-z][A-Za-z'’._\/ -]*)*\s*\)//i;
  $rest =~ s/\s+/ /g;
  $rest =~ s/^\s+|\s+$//g;
